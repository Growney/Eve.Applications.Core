...
namespace Eve.ESI.Standard.Authentication.Client
{
    public interface IESIAuthenticationClient
    {
        ...
        Task<ESITokenRefreshResponse> RefreshToken(string refreshToken); Task<T> GetSingleResponseAsync<T, K>(ESICallParameters parameters)
             where T : ESICallResponse<K>, new()
             where K : ESIItemBase;
        Task<T> GetCollectionResponseAsync<T, K>(ESICallParameters parameters)
            where T : ESICollectionCallResponse<K>, new()
            where K : ESIItemBase;
        Task<T> GetIntegerCollectionResponse<T, K>(ESICallParameters parameters)
